...
case 1
   ...
   tmp1=abs((0.25+ f).^(0.5));
case 2
   tmp1=e.^-(((j'-0.5)*29).^2*ones(size(j))+ones(size(i'))*((i-0.5)*29).^2);
   tmp1=e.^-(((j'-1.5)*29).^2*ones(size(j))+ones(size(i'))*((i-1.5)*29).^2);
   tmp1=e.^-(((j'-0.5)*29).^2*ones(size(j))+ones(size(i'))*((i-0.5)*29).^2);
   tmp1=e.^-(((j'-1.5)*29).^2*ones(size(j))+ones(size(i'))*((i-1.5)*29).^2);
case 3
   tmp=zero()
case 4
   tmp1=e.^-(((j'-1.0)*8).^2*ones(size(j))+ones(size(i'))*((i-0.5)*8).^2);
endswitch
